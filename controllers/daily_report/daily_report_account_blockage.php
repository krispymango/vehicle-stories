...
//vehicle_account_blocked_report
$sql_ab = "SELECT * FROM vw_dr_blocked";
$sql_ab_exec = mysqli_query($conn,$sql_ab);
...
if ($sql_ab_exec)
{
  $message_account_blocked_header = "
  <h4 style='text-align:center;'>Account blockage daily report</h4><table>
      <thead>
      <th>id</th>
    <th>username</th>
...
while ($sql_ab_fetch = mysqli_fetch_assoc($sql_ab_exec))
{
  $message_account_blocked_body .=
        "<tr>
          td>".$a++."</td>
          <td>".$sql_ab_fetch['username']."</td>
          <td>".$sql_ab_fetch['change_password_date']."</td>
          <td>".$sql_ab_fetch['change_password_time']."</td>
        </tr>";
}
$message_account_blocked_footer = "
    </tbody>
    </table>
    <br>";
}
