...
if (isset($_GET['acc_key']) && isset($_GET['u_id']))
{
  ...
  $sql = "SELECT * FROM vs_user WHERE id = '$u_iid' AND registration_activation_key = '$_GET[acc_key]' AND registration_key_active_status = 1";
  ...
  if ($sql_num_row && $sql_fetch)
  {
    ...
    $sql_upd = "UPDATE vs_user SET registration_key_active_status = 0,email_active = 0, registration_activation_key = '' WHERE id = '$u_iid' AND registration_activation_key = '$_GET[acc_key]'";
    ...
    if ($sql_upd_exec)
    {
      ...
      <div style='text-align: center;width:80%; margin:0px auto; margin-top:10vh;'>
        <h1><i style='color:green;' class='fas fa-lg fa-check-circle'></i> Your Account has been successfully activated!</h1>
      ...
      else
      {
        echo "
        <div style='text-align: center;width:80%; margin:0px auto; margin-top:10vh;'>
          <h1><i style='color:red;' class='fas fa-lg fa-exclamation-circle'></i> Error could not activate your account!</h1>
